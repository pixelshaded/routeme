...
function Router(abs_controller_folder, validationCB){
	
	if (abs_controller_folder === undefined){
		console.log('A folder path for controllers was not specified in the Routeme constructor.'.error);
		return;
	}
	
	var newObj = this;	
	this.routes = {};
	this.folder = abs_controller_folder;
	this.bound = false;
	...
	util.foreachFileInTreeSync(newObj.folder, function(folderPath, file){
		processController.call(newObj, folderPath, file);
	});
}

Router.prototype.generateURL = function(name, data){
...
Router.prototype.bindToExpress = function(app){
...
Router.prototype.printRouteMap = function(){
...
Router.prototype.validationErrorCallback = function(errors, req, res, next){
...
function processController(folderPath, file){
	...
	for (var i = 0; i < routes.length; i++){
		...
		var match = Router.prototype.findRouteByUri.call(this, route.uri, route.method);
