  ...
  (function () {
    ...
    Validation.prototype.addAttributeConstructor('format', function formatConstructor() {
      ...
      var formats = {
         ...
         */'date-time': {
          type: 'string',
          pattern: /^(\d{4})\D?(0[1-9]|1[0-2])\D?([12]\d|0[1-9]|3[01])(\D?([01]\d|2[0-3])\D?([0-5]\d)\D?([0-5]\d)?\D?(\d{3})?([zZ]|([+-])([01]\d|2[0-3])\D?([0-5]\d)?)?)?$/
        },
    ...
    var errorMessages = {
      ...
      pattern: function (property, propertyValue, attributeValue) {
        return 'The ‘' + property + '’ was not in a valid format.';
      },
    ...
    Validation.prototype.getProperty = function (property, source) {
      var tree = property.match(/([a-zA-Z0-9\s_]+)/g);
